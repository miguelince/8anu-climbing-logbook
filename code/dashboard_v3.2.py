...
app.layout = html.Div(style={'backgroundColor': 'white'},
                      children=[
                          ...
                          # 2nd Row - Header(summary of crag) + Dropdown menu
                          dbc.Row(children=[
...
def alert_creator(selected_year, hoverdata, is_open):
...
def bar_chart_seasons(selected_year, hoverdata):
    ...
    if hoverdata is None:  # when no point is selected on the map (no crag selected)
...
def data_table(selected_year, hoverdata):
    if hoverdata is None:  # when no point is selected on the map (no crag selected)
        # filtering by the year selected
...
def style_pie_charts(selected_year, all_rows_data, slctd_rows_indices, hoverdata):
    ...
    if hoverdata is None and (slctd_rows_indices is None or len(slctd_rows_indices) == 0):
    ...
    elif hoverdata is not None and (slctd_rows_indices is None or len(slctd_rows_indices) == 0):
        crag = hoverdata['points'][0]['customdata']  # extrating the name of the crag selected
    ...
    elif slctd_rows_indices is not None:
